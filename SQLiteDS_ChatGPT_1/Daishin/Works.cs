...
namespace SQLiteDS_ChatGPT_1.Daishin
{
    ...
    public class Works(FeedBus bus)
    {
        ...
        private async Task InsertOption()
        {
            ...
            try
            {
                ViewModel.AddLog("옵션 코드 입력 중...");

                if (OpCodes == null || OpCodes.Count == 0)
                {
                    ViewModel.AddLog("옵션 코드가 없습니다.");
                    return;
                }
                    ...
                    for (int i = 0; i < codeList.Count; i++)
                    {
                        ...
                        for (int c = 1; c < 3; c++)
                        {
                            ...
                            if (count < 364)
                            {
                                ...
                                {
                                    ViewModel.AddLog($"옵션 코드 서버 입력 실패({code}): {ex.Message}");
                                    continue;
                                }
                                count++;
                            }
            ...
            {
                ViewModel.AddLog($"서버에 옵션 코드 입력 중 에러 : {ex.Message}\n입력 개수 : {count}개");
            }
            ViewModel.AddLog("옵션 코드 입력 종료!");
        }
        private async Task<decimal> MstOption(string code, int index)
        {
            ...
            {
                ViewModel.AddLog($"옵션 현재가 받는 중 ㅇ에러 : {ex.Message}");
            }
            return result;
        }
        private async void OCur_Received()
        {
            try
            {
                int count = int.Parse(_view!.CurrentOpt.Replace(",", "")) + 1;
                _view.CurrentOpt = $"{count:N0}";
            ...
            {
                ViewModel.AddLog($"옵션 현재가 받는 중 에러 : {ex.Message}");
            }
        }
        ...
        private void InputServer()
        {
            try
            {
                ViewModel.AddLog("선물 코드 입력 시작");
                if (FuCodes == null || FuCodes.Count == 0)
                {
                    ViewModel.AddLog("선물 코드가 없습니다.");
                    return;
                }
                ...
                ViewModel.AddLog("선물 주간, 현재가 가저오기 및 코드 입력");
                ...
                ViewModel.AddLog("서버에 코드 입력 종료");
            }
            catch (Exception ex)
            {
                ViewModel.AddLog($"InputServer Error : {ex.Message}");
            }
        ...
        private void KCur_Received()
        {
            try
            {
                int count = int.Parse(_view!.Kos200.Replace(",", "")) + 1;
                _view.Kos200 = $"{count:N0}";
            ...
            {
                ViewModel.AddLog($"kospi200 지수 받는 중 에러 : {ex.Message}");
            }
        }
        private void SCur_Received()
        {
            ...
            {
                ViewModel.AddLog($"Kospi 지수 받는 중 에러 : {ex.Message}");
            }
        }
        private async void StockData()
        {
            try
            {
                ...
                foreach (string code in codes)
                {
                        ...
                        d == int.Parse(DateTime.Today.ToString("yyyyMMdd")))
                    {
                        for (int i = 2; i < 6; i++)
                        {
                            decimal close = ComHelpers.AsDecimal(cls.GetDataValue(i, 0));
                            decimal past = ComHelpers.AsDecimal(cls.GetDataValue(5, 0)) -
            ...
            {
                ViewModel.AddLog($"업종 지수를 받는 중 에러 : {ex.Message}");
            }
        }
        private async void GetDataMst(string code)
        {
            try
            {
                ...
                System.Windows.Application.Current.Dispatcher.Invoke(() =>
                {
                    int count = int.Parse(_view!.CurrentFut.Replace(",", "")) + 1;
                    _view!.CurrentFut = $"{count:N0}";
                });
            }
            catch (Exception ex)
            {
                ViewModel.AddLog($"선물 현재가 받는 중 에러 : {ex.Message}");
            }
        }
        private void FBid_Received()
        {
            try
            {
                int count = int.Parse(_view!.BidNask.Replace(",", "")) + 1;
                _view!.BidNask = $"{count:N0}";
            ...
            {
                ViewModel.AddLog($"선물 호가 받는 중 에러 : {ex.Message}");
            }
        }
        ...
        private void FExc_Received()
        {
            try
            {
                int count = int.Parse(_view!.ExpectFut.Replace(",", "")) + 1;
                string str = $"{count:N0}";
                _view!.ExpectFut = str;
            ...
            {
                ViewModel.AddLog($"선물 예상가 받는 중 에러 : {ex.Message}");
            }
        }
        ...
        {
            ViewModel.AddLog("투자자별 매매종합 작업 시작.");
            try
            {
            ...
            {
                ViewModel.AddLog($"투자자별 매매 종합 작업 중 에러 : {ex.Message}");
            }
        }
        private void C7721s_Received()
        {
            int count = 0;
            if (_view!.Trading != string.Empty)
                count = int.Parse(_view!.Trading.Replace(",", ""));
            try
            {
                count++;
                _view.Trading = $"{count:N0}";
                // 0: 개인, 1: 외국인, 2: 기관
                for (int i = 0; i < 3; i++)
                {
                    Trading trade = new()
            ...
            {
                ViewModel.AddLog($"투자자별 매매 현황 접수 중 에러 : {ex.Message}");
            }
        }
        private async void FutureHighLow(string code)
        {
            ...
            {
                ViewModel.AddLog($"선물 고가 저가 입력 중 에러 : {ex.Message}");
            }
        }
        private void KospiHighLow(string code = "U180")
        {
            ViewModel.AddLog("코스피 주간 가격 작업 시작.");
            try
            {
            ...
            {
                ViewModel.AddLog($"KospiHighLow 작업 중 에러 발생 : {ex.Message}");
            }
        }
        ...
        private void GetOptionCode()
        {
            ...
            {
                ViewModel.AddLog($"GetOptionCode Error : {ex.Message}");
            }
        }
        private void GetFutureCode()
        {
            ...
            {
                ViewModel.AddLog($"GetFutureCode Error : {ex.Message}");
            }
        }
        public void Dispose()
        {
