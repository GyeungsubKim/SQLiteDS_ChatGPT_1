...
{
    internal class CybosConnection : IDisposable
    {
        ...
        internal async Task WaitTime(string funName)
        {
            ...
            {
                ViewModel.AddLog($"{funName} 서버대기 : " + DateTime.Now.ToString("HH:MM:ss") + ", " +
        ...
        {
            ViewModel.AddLog($"{DateTime.Now:yyyy년 MM 월 dd일} 선물 옵션 데이터 작업 시작");
            ...
            try
            {
                ...
                ViewModel.AddLog($"시장 시작 시간 : {open:HHmm}, 마감 시간 : {close:HHmm}");
            }
            catch (Exception ex)
            {
                ViewModel.AddLog($"시장 시간 가져오기 Error : {ex.Message}");
            }
            return (open, close);
        }
